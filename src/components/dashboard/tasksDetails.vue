<template>
  <UiKitsUiSlotsDashboardSlot v-if="task">
    <div class="flex flex-col">
      ...
      <div class="flex items-center justify-between my-5">
        <div class="flex items-center gap-2 h-[40px] rounded-lg bg-gray-100 text-gray-400 p-5">
          <img src="/assets/icons/bellIcon.svg" class="w-5 h-5" alt="Notification Icon" />
          <p>
            {{ texts.deadline }}
        ...
        <div>
          <button class="relative py-1 rounded-full w-[60px] h-[60px] text-[#000] font-semibold progressTasksColor"
            :style="{
      ...
      <div class="flex items-center justify-start gap-5">
        <img src="/assets/icons/editIcon.svg" alt="edit" class="cursor-pointer w-4 h-4" title="Edit"
          @click="editTaskModal" />
        <UDropdown mode="hover" :popper="{ placement: 'right-start' }" :items="deleteLists">
          <img src="/assets/icons/deleteIcon.svg" alt="edit" class="cursor-pointer w-4 h-4" title="Delete" />
        </UDropdown>
      </div>
    </div>
  ...
  <!-- Edit Task Modal -->
  <UiKitsUiSlotsFormModelSlot form-title="Edit Task" @close-modal="editTaskModal" v-if="createATasks"
    v-model="createATasks" @closeDialog="editTaskModal">
    ...
    <div class="flex justify-end">
      <UButton class="w-fit" color="blue" variant="solid" @click="editATaskSubmit">{{ texts_a.buttonEdit }}</UButton>
    </div>
